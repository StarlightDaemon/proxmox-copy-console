...
OL-04 latest: candidate commit `6d803b0` passed CI. The maintainer reports **Copy worked** after the candidate-only reload request; the new log has no probe output, but ExtJS `fireFn`/`timerId` and null-controller `loadTags` exceptions persist. Promotion is on hold pending a scripts-disabled baseline to isolate those exceptions and the Firefox + Violentmonkey regression check. See Testing for evidence limits. No further speculative runtime patch is queued.
...
| OL-04 | Extended / candidate awaiting live acceptance | FireMonkey revision-2 report establishes callback-cloning and page-array filter/some failures, with discovery and owner/frame checks passing. Owner `/root` prepared shared-source 0.4.1-dev.1 on `codex/firemonkey-compat`; 88 local tests pass. Next: FireMonkey node/LXC copying, then Firefox + Violentmonkey primary regression and Tampermonkey. Stable main remains 0.4.0; no FireMonkey pass claimed. Greasemonkey and ScriptCat remain future Firefox checks; Chrome checks follow later. A separate manager edition is unnecessary for this candidate. |
