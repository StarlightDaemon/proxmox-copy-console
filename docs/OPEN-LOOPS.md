...
| OL-04 | Extended / FireMonkey activation unresolved | FireMonkey 3.8 attempt on 2026-09-19 reportedly does not work; screenshot shows Other Scripts only, with active-tab context unconfirmed. Follow the activation check in Testing before treating this as a runtime defect. Greasemonkey and ScriptCat remain next Firefox checks. Record each result separately. Chrome/OrangeMonkey checks are deferred for this round. Add an adapter or generated metadata variant only for a demonstrated incompatibility with a regression case; otherwise keep the shared script. Generic injectors and Safari remain outside the implementation plan. |
