...
OL-04 latest: candidate commit `6d803b0` passed CI. The maintainer reports **Copy worked** with the probe disabled. Both ExtJS `fireFn`/`timerId` and null-controller `loadTags` exceptions also appear in the log supplied for the scripts-disabled baseline; this suggests an independent issue, with cause undiagnosed. See Testing for the reported-run evidence boundary. Next is the Firefox + Violentmonkey regression check before promotion; no further speculative runtime patch is queued.
...
| OL-02 | Both primary Firefox managers passed / evidence follow-up | Maintainer reports successful dev.2 testing with Violentmonkey 2.49.0 and Tampermonkey 5.5.0 in the Firefox 156.0 (64-bit) / Proxmox VE 9.2.3 round. Tampermonkey is reported fully tested and working with no issues. Preserve these practical passes; remaining detailed qualification can record `pve-xtermjs` version, probes, and per-scenario results without inventing observations. See Testing. |
...
| OL-04 | Extended / FireMonkey copying works; primary regression pending | Owner `/root` prepared shared-source 0.4.1-dev.1 on `codex/firemonkey-compat` for observed callback-boundary failures; 88 local tests and CI pass. Maintainer reports FireMonkey copying success. UI exceptions also recur in the supplied scripts-disabled baseline; no script-specific cause established. Next: Firefox + Violentmonkey primary regression, then Tampermonkey; detailed FireMonkey coverage remains unrecorded. Stable main remains 0.4.0. Greasemonkey and ScriptCat remain future Firefox checks; Chrome checks follow later. No separate manager edition is needed so far. |
