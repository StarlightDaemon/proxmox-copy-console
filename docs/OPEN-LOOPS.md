...
| OL-04 | Extended / FireMonkey integration failure unresolved | Maintainer confirms FireMonkey 3.8 Copy button absent on both node Shell and container Console on 2026-09-19. Probe confirms startup and required API/global presence; original script emits an integration warning. Upstream popup matcher strips ports, explaining misleading Other Scripts classification. Collect revision-2 manager probe's delayed integration report to isolate failing calls; see Testing. Greasemonkey and ScriptCat remain next Firefox checks. Chrome/OrangeMonkey checks are deferred for this round. Add an adapter or generated metadata variant only for a demonstrated incompatibility with a regression case; otherwise keep the shared script. Generic injectors and Safari remain outside the implementation plan. |
