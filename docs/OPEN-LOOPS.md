...
OL-04 latest: candidate commit `6d803b0` passed CI. The maintainer reports FireMonkey copying success, successful Firefox + Violentmonkey navigation/copying, Greasemonkey 4.14 functional success, and now a Chrome + Tampermonkey workflow pass. Earlier ExtJS exceptions remain undiagnosed; the Chrome screenshot contains startup messages and form/unload notices without those exceptions visible. Next are Chrome + Violentmonkey and the Firefox + Tampermonkey candidate regression. See Testing for evidence boundaries; no further speculative runtime patch is queued.
...
| OL-03 | Active / Chrome Tampermonkey workflow passed | Maintainer resumed operator testing after canceling computer control. Candidate 0.4.1-dev.1 passes the reported Chrome + Tampermonkey workflow; exact browser/manager versions remain requested. Next: Chrome + Violentmonkey with the same pinned source. Complete both primary rows with actual clipboard comparisons and detailed scenario evidence before claiming full qualification; no agent-run clipboard pass occurred. |
| OL-04 | Extended / three Firefox managers functionally passed | Owner `/root` prepared shared-source 0.4.1-dev.1 on `codex/firemonkey-compat`; 88 local tests and CI pass. Maintainer reports FireMonkey copying success, Firefox + Violentmonkey shell/container copying with dynamic navigation working without issues, and Greasemonkey 4.14 fully functional. Recurring UI exceptions with other extensions present remain in OL-10. Next: Firefox + Tampermonkey candidate regression; detailed extended-manager coverage remains unrecorded. Stable main remains 0.4.0. ScriptCat remains a future Firefox check; current Chrome evidence is in OL-03. No separate manager edition is needed so far. |
