...
OL-04 latest: candidate commit `6d803b0` passed CI. The maintainer reports FireMonkey copying success, successful Firefox + Violentmonkey navigation/copying, Greasemonkey 4.14 functional success, and workflow passes with both primary Chrome managers. Earlier ExtJS exceptions remain undiagnosed; the Chrome Issues screenshots show form/unload notices that do not warrant candidate changes. Next is the Firefox + Tampermonkey candidate regression. See Testing for version and scenario evidence boundaries; no further speculative runtime patch is queued.
