...
OL-04 latest: candidate commit `6d803b0` passed CI. The maintainer reports copying success in FireMonkey and a successful Firefox + Violentmonkey regression across shells, container consoles, and dynamic navigation. Earlier ExtJS exceptions appeared in the supplied scripts-disabled baseline and are absent from the latest Violentmonkey log; their cause remains undiagnosed. Next are candidate Tampermonkey regression and later Chrome checks. See Testing for evidence boundaries; no further speculative runtime patch is queued.
...
| OL-04 | Extended / FireMonkey copying and primary regression passed | Owner `/root` prepared shared-source 0.4.1-dev.1 on `codex/firemonkey-compat`; 88 local tests and CI pass. Maintainer reports FireMonkey copying success and Firefox + Violentmonkey shell/container copying with dynamic navigation working without issues. Next: candidate Tampermonkey regression; detailed FireMonkey coverage remains unrecorded. Stable main remains 0.4.0. Greasemonkey and ScriptCat remain future Firefox checks; Chrome checks follow later. No separate manager edition is needed so far. |
