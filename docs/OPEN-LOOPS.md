...
| OL-04 | Extended / FireMonkey missing button unresolved | Maintainer confirms FireMonkey 3.8 Copy button absent on both node Shell and container Console on 2026-09-19. Upstream popup matcher strips ports; local reproduction explains misleading Other Scripts classification but does not establish why integration fails. Collect the existing manager probe output and registration errors as described in Testing. Greasemonkey and ScriptCat remain next Firefox checks. Chrome/OrangeMonkey checks are deferred for this round. Add an adapter or generated metadata variant only for a demonstrated incompatibility with a regression case; otherwise keep the shared script. Generic injectors and Safari remain outside the implementation plan. |
