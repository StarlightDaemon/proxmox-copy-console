...
Next live checks: Chrome + Violentmonkey with this same candidate, and the outstanding Firefox + Tampermonkey candidate regression. Chrome + Tampermonkey now has the maintainer-reported workflow pass below. Keep one manager enabled at a time and preserve trusted-host restrictions. Chromium derivatives remain expected-compatible but unverified, without a routine broad test matrix.
